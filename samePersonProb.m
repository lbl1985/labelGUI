function varargout = samePersonProb(varargin)
% SAMEPERSONPROB MATLAB code for samePersonProb.fig
%      SAMEPERSONPROB, by itself, creates a new SAMEPERSONPROB or raises the existing
%      singleton*.
%
%      H = SAMEPERSONPROB returns the handle to a new SAMEPERSONPROB or the handle to
%      the existing singleton*.
%
%      SAMEPERSONPROB('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SAMEPERSONPROB.M with the given input arguments.
%
%      SAMEPERSONPROB('Property','Value',...) creates a new SAMEPERSONPROB or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before samePersonProb_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to samePersonProb_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help samePersonProb

% Last Modified by GUIDE v2.5 17-May-2012 13:53:26

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @samePersonProb_OpeningFcn, ...
                   'gui_OutputFcn',  @samePersonProb_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before samePersonProb is made visible.
function samePersonProb_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to samePersonProb (see VARARGIN)

handles.Person = [];
% Choose default command line output for samePersonProb
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes samePersonProb wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = samePersonProb_OutputFcn(hObject, eventdata, handles) 
...
% --- Executes on button press in pbPlot1.
function pbPlot1_Callback(hObject, eventdata, handles)
% hObject    handle to pbPlot1 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if ~isempty(handles.cam1.tmpPerson)
    tmpPerson = handles.cam1.tmpPerson;
    nRegion = length(tmpPerson.tAll);
    score = zeros(nRegion, nRegion);
    for i = 1 : 2 : nRegion
        refI = read(handles.cam1.obj, tmpPerson.tAll(i));
        refRegion = rectroi(refI, tmpPerson.rectAll(i, [2 1 4 3]));
%         figure(1); subplot(1, 2, 1); imshow(refRegion); title('refRegion');
        refRCObj = RCFlipInvariant(refRegion, [1 1 tmpPerson.rectAll(i, [4 3])]);
        refRCObj.calCovariance;
        left = [1 : 2 : i - 1 i+1 : 2 : nRegion];
        for j = left
            tgtI = read(handles.cam1.obj, tmpPerson.tAll(j));
            tgtRegion = rectroi(tgtI, tmpPerson.rectAll(j, [2 1 4 3]));
%             figure(1); subplot(1, 2, 2); imshow(tgtRegion); title('tgtRegion');
            tgtRCObj = RCFlipInvariant(tgtRegion, [1 1 tmpPerson.rectAll(j, [4 3])]);
            tgtRCObj.calCovariance;
            RCCompareObj = RCCompare(refRCObj.covariance, tgtRCObj.covariance);
            score(i, j) = RCCompareObj.compare();
%             score
        end
        display(num2str(i/nRegion));
    end
    scoreList = score(:); 
    scoreList = scoreList(scoreList ~= 0);
    figure(3); hist(scoreList, 0: 0.5 : 10); title(['person ' num2str(handles.cam1.currSelection) ...
    ' in camera ' handles.cam1.VideoName]);
end


% --- Executes on button press in pbPlot2.
function pbPlot2_Callback(hObject, eventdata, handles)
% hObject    handle to pbPlot2 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if ~isempty(handles.cam2.tmpPerson)
    tmpPerson = handles.cam2.tmpPerson;
    nRegion = length(tmpPerson.tAll);
    score = zeros(nRegion, nRegion);
    for i = 1 : 2 : nRegion
        refI = read(handles.cam2.obj, tmpPerson.tAll(i));
        refRegion = rectroi(refI, tmpPerson.rectAll(i, [2 1 4 3]));
%         figure(1); subplot(1, 2, 1); imshow(refRegion); title('refRegion');
        refRCObj = RCFlipInvariant(refRegion, [1 1 tmpPerson.rectAll(i, [4 3])]);
        refRCObj.calCovariance;
        left = [1 : 2 : i - 1 i+1 : 2 : nRegion];
        for j = left
            tgtI = read(handles.cam2.obj, tmpPerson.tAll(j));
            tgtRegion = rectroi(tgtI, tmpPerson.rectAll(j, [2 1 4 3]));
%             figure(1); subplot(1, 2, 2); imshow(tgtRegion); title('tgtRegion');
            tgtRCObj = RCFlipInvariant(tgtRegion, [1 1 tmpPerson.rectAll(j, [4 3])]);
            tgtRCObj.calCovariance;
...
%             score
    ...
    scoreList = score(:); 
    scoreList = scoreList(scoreList ~= 0);
    figure(3); hist(scoreList, 0: 0.5 : 10); title(['person ' num2str(handles.cam2.currSelection) ...
    ' in camera ' handles.cam2.VideoName]);
end
